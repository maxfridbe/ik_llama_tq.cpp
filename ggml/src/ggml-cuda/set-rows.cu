...
// ── Sequential (non-scatter) turbo quantize for ggml_cpy ─────────────────────
// Called when ggml_cpy(f32_src, turbo3_dst) with no index tensor.
// One thread block per WHT group; thread j handles element j in that group.
...
// Dispatch f32 → turbo3/2/4 via sequential indices (for ggml_cpy use)
void ggml_cuda_cpy_f32_to_turbo(
        ggml_backend_cuda_context & ctx,
        const ggml_tensor * src0,
        ggml_tensor       * dst) {
    ...
    // Allocate sequential index buffer
    ...
    // Build a fake I32 index tensor wrapping idx_buf
